    ...
    <div className="App">
      ...
      <main className="login-container">
        <div className="unique-feature">
          <img src="login image.png?v=2" alt="Unique Feature" className="unique-feature-image" />
        </div>
            ...
            {!otpSent ? (
              <form onSubmit={handleLogin}>
                ...
                <input
                  ...
                  required
                />
                <label htmlFor="mobile">Mobile Number <i>*</i></label>
                <div className="mobile-input-container">
                  ...
                  <span
                    ...
                    tabIndex={0} // Makes the element focusable by keyboard navigation
                    role="button" // Makes it behave like a button
                    aria-pressed={isMobileVisible} // Indicates whether the element is pressed or not
                    onKeyDown={(e) => {
                      // Ensure the element can be activated using keyboard
                      if (e.key === 'Enter' || e.key === ' ') {
                        e.preventDefault(); // Prevent default to avoid page scroll on Space key
                        setIsMobileVisible(!isMobileVisible);
                      }
                    }}
                    onTouchEnd={() => setIsMobileVisible(!isMobileVisible)} // Allows touch interaction
                  >
                    {isMobileVisible ? <FaEyeSlash /> : <FaEye />}
                  </span>
                </div>
                <label htmlFor="employeeid">Emp ID/Client ID <i>*</i></label>
                <input
                  type="text"
                  id="employeeid"
                ...
                {chatMessages.map((message) => (
                  <div key={message.id} className={`message ${message.sender}`}>
                    ...
                    <div className="message-time">
                      {messageTimestamps[chatMessages.indexOf(message)].toLocaleTimeString([], {
                        hour: '2-digit',
                        minute: '2-digit',
                      })}
                    </div>
