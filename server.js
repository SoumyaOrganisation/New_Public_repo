...
const storage = multer.diskStorage({
  ...
  filename: function (req, file, cb) {
    // We'll set the filename after we get the form fields
    cb(null, 'temp.jpg');
  }
});
...
app.post('/upload-profile-image', (req, res) => {
  upload(req, res, async function(err) {
    ...
    try {
      ...
      const userName = req.body.userName.replace(/[^a-zA-Z0-9]/g, '_');
      
      // Rename the temp file to include the username
      const tempPath = req.file.path;
      const targetPath = path.join(uploadDir, `${userName}.jpg`);

      fs.renameSync(tempPath, targetPath);

      logger.info(`Profile image uploaded successfully for user: ${userName}`);
...
app.get('/get-profile-image/:name', (req, res) => {
  try {
    const userName = req.params.name.replace(/[^a-zA-Z0-9]/g, '_');
    const imagePath = path.join(__dirname, 'uploads\\profile-images', `${userName}.jpg`);
    
    logger.info(`Attempting to load profile image for: ${userName}`);
    ...
    if (fs.existsSync(imagePath)) {
      ...
      logger.info(`No profile image found for ${userName}`);
